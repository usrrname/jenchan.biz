...
I landed at a health tech company whose forte was clinical data storage but they were trying to make headway on health apps. On one of their projects, my team was tasked to rewrite a legacy .NET surgery prioritization system into an Angular 9 app for a provincial health organization. Their primary users would be hospital admin staff.

The app would take a combination of answers entered into an 8-part form with 120 fields to assess how soon a patient had to undergo a surgery for a health condition.
...
Form state could only be inspected at runtime by console logging different lines within functions, but not in any devtool.
