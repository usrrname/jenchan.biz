...
draft: false
summary: Expectations crash into reality while trying to turn prompts into workflows.
tags: ["AI", "cursor", "agentic", "vibe coding"]
images: ["/static/images/blog/agentic-ai-workflow-woes/woman-yawning-with-dog.jpg"]
date: 2025-04-27
---

Ubiquity has the effect of making certain tools sound way more effective than they are. If everyone and their mom is using AI to ship faster, I too must partake!

![Screenshot of Twitter user @joshycodes](https://i.redd.it/l61y61w2mxve1.jpeg)

In this shitpost, I'll share some of the drudgery I've run into with vibe coding and trying to build an "agentic" workflow with Cursor.

TLDR; Workflows where the LLM-assisted agent is highly possible, but require a LOT of config, oversight and tuning to keep on track. And depending which model you choose, the quality and precision of execution can vary. 
...
...with chat-oriented programming.
...
# npx cursorrules --flat if you're already in a project directory
```
...
![Screenshot of cursor chat window](/static/images/blog/agentic-ai-workflow-woes/godmode-chat.png)
...
The text was looking rather faint, so I asked it to make the text contrast ratio more accessible (4:2:1).

![Screenshot of vibe coded Pokemon cards containing AI agents](/static/images/blog/agentic-ai-workflow-woes/vibe-coding-1.png)
...
The desired outcome would be ahighly controlled series of task breakdowns with many checkpoints to keep it on course, like reviewing student work that doesn't truly understand and easily flies off the rails.
...
I tried creating a rule that would log the context size and number of remaining requests on the Pro subscription, but it was wildly inaccurate. Natural language rule quirk: it appears certain keywords will export read-only values when interpolated into a cursor rule, for example, `{context_size}`.[^4] 

The markdown-adjacent `.mdc` format of Cursor Rules also seems to support Javascript template literals, so this is a readability gotcha when you have rules that look like this: 
...
actions:
  - type: append
    content: |
      ...
      Usage-based Pricing: {usage_pricing_enabled}       // read-only value returned from Cursor backend
      Monthly Requests: {calculateUsage().requests}/{calculateUsage().maxRequests}

      Model Usage: {model_name}  // read-only value returned from Cursor backend, though not always accurate
      {formatModelUsage(calculateUsage().modelUsage)}

      Context size: ~{context_size}k tokens {context_size > 40 ? "(Large Context Mode)" : ""}
      Max Context Size: {context_window_size} // read-only value returned from Cursor backend
```

Right now,there is no public API to check the history of requests, and this is rather important for cost-conscious startups, freelancers, and any cost-conscious dev who's funemployed... well basically every company that wants to do more with less. 

Meanwhile, Cline has this feature baked right in! 
...
For now, I'll settle with using this project as a config palette of scaffolding, standards and formatting opinions instead of a workflow automator.
