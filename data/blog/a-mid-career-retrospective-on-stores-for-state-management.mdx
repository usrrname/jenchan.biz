---
title: "A mid-career survey of stores for state management" 
draft: false
...
- [State management, but in moderation](#state-management-but-in-moderation)
...
At the time, Angular didn't have standalone components and we didn't know about Angular Universal's capabilities of SSR.[^3] We made multiple API calls sequentially to fetch and populate form questions and previous answers after login, instead of rendering the form statically.
...
<caption>VueX itself is particularly verbose with introducing the need to commit mutations before dispatching actions.</caption>
...
It can be tempting to create a store for every feature of an app too soon. This can result in unnecessary complexity or abstraction that's easy to overlook by newer devs when they're not famliar. 

Premature codesplitting confuses the team when the data source is the same, resulting in higher cognitive overhead to keep more stores' state updated, especially if they share the same data source.
...
1. There is complex domain-specific logic that needs to be diligently updated as part of a larger product or platform as a user navigates through different views.
  
2. It's become untenable to manage state within one or two components and you find that you're requesting or passing the same data properties across multiple components. (aka. [prop-drilling](https://kentcdodds.com/blog/prop-drilling))

3. There's a need to present or update different types of data across different features of an app, or work with nested deep data structures to update the UI.

4. If there are multiple dimensions of state to track, then a store might not give you enough granularity, and it may be worth looking at implementing a state machine (some libraries include [XState](https://xstate.js.org/) or [Immer](https://immerjs.github.io/immer/))
   
For example, if you need interaction changes through multi-step forms, wizards or like the combinatories of form fields I previously described. _Caveat: I have not had to use above libs yet._

## State management, but in moderation

Front end frameworks today have more lightweight and facile options to manage state.
...
React hooks and [Context API](https://legacy.reactjs.org/docs/context.html) vastly reduced the need for prop drilling by being able to refer to context values anytime at any level of view depth within a component. 
...
Vue's `computed()`, `ref()` and `watch()` functions and composables allow us to accomplish the same things as React hooks and context. 

And even newer, sexier options: [signals](https://github.com/tc39/proposal-signals) in Svelte and Angular, will be making it into Javascript.

The problems I see frameworks solving for are the same: 
- the need to reactively update the UI when state changes, 
- to keep the UI in sync with data fetched from other sources. 
- and for the neurotic folks, to draw clean lines between state changes that should be local to components, shared across components, or global to the app.
